...
struct GraphQLAstNode *graphql_parse_string(const char *text, const char **error) {
  return reinterpret_cast<struct GraphQLAstNode *>(lnear::graphql::parseString(text, error).release());
}

struct GraphQLAstNode *graphql_parse_string_with_experimental_schema_support(
    const char *text, const char **error) {
  return reinterpret_cast<struct GraphQLAstNode *>(lnear::graphql::parseStringWithExperimentalSchemaSupport(
                                                     text, error).release());
}

struct GraphQLAstNode *graphql_parse_file(FILE *file, const char **error) {
  return reinterpret_cast<struct GraphQLAstNode *>(lnear::graphql::parseFile(file, error).release());
}

struct GraphQLAstNode *graphql_parse_file_with_experimental_schema_support(
    FILE *file, const char **error) {
  return reinterpret_cast<struct GraphQLAstNode *>(lnear::graphql::parseFileWithExperimentalSchemaSupport(file, error).release());
}
